...
const login = catchAsync(async (req, res, next) => {
  ...
  if (!updateRefreshToken){
    await RefreshToken.create({ username: user.username, token: refreshToken })
  }
...
const restricTo = (permissionName, val) => {
...
// check permission
const checkPermmision = catchAsync(async (req, res, next) => {
  

  const permissionName = req.params.perm
