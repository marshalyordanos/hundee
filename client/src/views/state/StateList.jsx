...
const StateList = () => {
  ...
  const handlePagination = async (page, pageSize) => {
    ...
    // if (!(await authService.checkPermmision('state', 'read'))) {
    //     return message.error('You have not a permmission');
    // }

    setSearchParams({ page: page, limit: pageSize });
    dispatch(updateStateState({ page: page, limit: pageSize }));
  ...
  const tableChange = (pagination, filters, sorter) => {
    ...
    dispatch(updateStateState({ sort: field, order: order }));
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      const data = await stateService.deleteStat(modeID);
    ...
    <div>
      ...
      <span className="flex md:flex-row flex-col justify-between items-start md:items-end borde border-rose-700">
        <div className="flex flex-col p-6 md:w-[45vw] w-full">
          <h1 className="text-2xl font-bold pb-4">States</h1>
          <div className="flex">
            <Input
